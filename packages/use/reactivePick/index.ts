...
import type { ReactivePickPredicate, ReactivePickReturn } from "../types";
...
function createPickedObject<T extends object>(
	obj: T,
...
function createPredicatePickedObject<T extends object>(
	obj: T,
...
): ReactivePickReturn<T>;

export function reactivePick<T extends object, K extends keyof T>(
	obj: T,
	...keysOrPredicate: readonly (K | readonly K[] | ReactivePickPredicate<T>)[]
): ReactivePickReturn<T, K> {
	...
	if (typeof first === "function") {
		...
		return reactiveComputed(() =>
			createPredicatePickedObject(obj, (value, key) =>
				predicate(value as Parameters<ReactivePickPredicate<T>>[0], key),
			),
		) as ReactivePickReturn<T, K>;
	}

	const keys = keysOrPredicate.flatMap((key) =>
		toArray(key as K | readonly K[]),
	);
	...
	return reactiveComputed(() =>
		createPickedObject(obj, uniqueKeys),
	) as ReactivePickReturn<T, K>;
}
